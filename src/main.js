import * as core from '@actions/core'
import * as tc from '@actions/tool-cache'
import * as exec from '@actions/exec'
import * as path from 'path'
import * as fs from 'fs'
import * as os from 'os'

/*
Credits:
https://github.com/warrenbuckley/Setup-Nuget
https://github.com/warrenbuckley/Setup-MSBuild
https://github.com/Malcolmnixon/Setup-VSTest
*/

const net45SdkUrl = 'https://www.nuget.org/api/v2/package/Microsoft.NETFramework.ReferenceAssemblies.net45/1.0.3'

const nugetVersion = 'latest'
const nugetUrl = `https://dist.nuget.org/win-x86-commandline/${nugetVersion}/nuget.exe`

const vsWhereVersion = '3.1.7'
const vsWhereUrl = `https://github.com/microsoft/vswhere/releases/download/${vsWhereVersion}/vswhere.exe`

async function SetupNet45Sdk() {
    // Download latest Nuget.exe
    core.info(`Download ${net45SdkUrl}`)
    const sdkZip = await tc.downloadTool(net45SdkUrl)
    core.info(`Save to ${sdkZip}`)

    const tempFolder = path.join(os.tmpdir(), 'net45sdk')
    const extractPath = await tc.extractZip(sdkZip, tempFolder)
    core.info(`Extract to ${extractPath}`)

    const net45SdkPath = `${extractPath}\\build\\.NETFramework\\v4.5`
    const sysSdkPath = 'C:\\Program Files (x86)\\Reference Assemblies\\Microsoft\\Framework\\.NETFramework\\v4.5'
    core.info(`Copy net45 SDK`)
    core.info(`From ${net45SdkPath}`)
    core.info(`to ${sysSdkPath}`)

    // Do not use fs.mv()!
    // Temp folder and system SDK folder may be in different drivers.
    fs.cpSync(net45SdkPath, sysSdkPath, { recursive: true, force: true, })
}

async function DownloadNuget() {
    // Download latest Nuget.exe
    core.debug('Downloading Nuget tool')
    const nugetPath = await tc.downloadTool(nugetUrl)

    // Rename the file which is a GUID without extension
    let folder = path.dirname(nugetPath)
    let fullPath = path.join(folder, 'nuget.exe')
    fs.renameSync(nugetPath, fullPath)

    //Cache the directory with Nuget in it - which returns a NEW cached location
    let cachedToolDir = await tc.cacheDir(folder, 'nuget', nugetVersion)
    core.debug(`Cached Tool Dir ${cachedToolDir}`)

    // Add Nuget.exe CLI tool to path for other steps to be able to access it
    core.addPath(cachedToolDir)

    return cachedToolDir
}

async function DownloadVsWhere() {
    core.debug(`Downloading VSWhere ${vsWhereVersion} tool`)
    const vsWherePath = await tc.downloadTool(vsWhereUrl)

    // Rename the file which is a GUID without extension
    let folder = path.dirname(vsWherePath)
    let fullPath = path.join(folder, 'vswhere.exe')
    fs.renameSync(vsWherePath, fullPath)

    //Cache the directory with VSWhere in it - which returns a NEW cached location
    return await tc.cacheDir(folder, 'vswhere', vsWhereVersion)
}

function CreateAddToPathHelper(vswhereLocation) {
    let vw = vswhereLocation
    return async function (tag, handler) {
        let path = await handler(vw)
        core.info(`Add ${tag} path ${path}`)
        core.addPath(path)
    }
}

async function FindMSBuild(pathToVSWhere) {
    let msBuildPath = ''

    const options = {}
    options.listeners = {
        stdout: data => {
            let output = data.toString()
            msBuildPath += output
        }
    }

    // Run VSWhere to tell us where MSBuild is
    let vsWhereExe = path.join(pathToVSWhere, 'vswhere.exe')
    await exec.exec(
        vsWhereExe,
        [
            '-latest',
            '-requires',
            'Microsoft.Component.MSBuild',
            '-find',
            'MSBuild\\**\\Bin\\MSBuild.exe'
        ],
        options
    )

    if (msBuildPath === '') {
        core.setFailed('Unable to find MSBuild.exe')
    }

    let folderForMSBuild = path.dirname(msBuildPath)
    core.debug(`MSBuild = ${msBuildPath}`)
    core.debug(`Folder for MSBuild ${folderForMSBuild}`)

    return folderForMSBuild
}

async function FindVSTest(pathToVSWhere) {
    let vsTestPath = ''

    const options = {}
    options.listeners = {
        stdout: data => {
            let output = data.toString()
            vsTestPath += output
        }
    }

    // Run VSWhere to tell us where VSTest.Console is
    let vsWhereExe = path.join(pathToVSWhere, 'vswhere.exe')

    // https://github.com/Malcolmnixon/Setup-VSTest/pull/3/commits/a773224064f250e2c6d53ad44764727a7dcea5e4
    await exec.exec(
        vsWhereExe,
        [
            '-latest',
            '-products',
            '*',
            '-requires',
            'Microsoft.VisualStudio.Workload.ManagedDesktop',
            'Microsoft.VisualStudio.Workload.Web',
            '-requiresAny',
            '-property',
            'installationPath'
        ],
        options
    )

    if (vsTestPath === '') {
        core.setFailed('Unable to find VSTest.Console.exe')
    }

    // https://github.com/Malcolmnixon/Setup-VSTest/pull/3/commits/a773224064f250e2c6d53ad44764727a7dcea5e4
    vsTestPath = path.join(
        vsTestPath.trimEnd(),
        '\\Common7\\IDE\\CommonExtensions\\Microsoft\\TestWindow\\vstest.console.exe'
    )

    let folderForVSTest = path.dirname(vsTestPath)
    core.debug(`VSTest = ${vsTestPath}`)
    core.debug(`Folder for VSTest ${folderForVSTest}`)

    return folderForVSTest
}

async function Main() {
    if (!core.platform.isWindows) {
        core.setFailed('This action only works for Windows.')
        return
    }

    try {
        await SetupNet45Sdk()

        const nugetLocation = tc.find('nuget', nugetVersion) || (await DownloadNuget())
        core.info(`Found nuget at ${nugetLocation}`)

        const vsWhereLocation = tc.find('vswhere', vsWhereVersion) || (await DownloadVsWhere())
        core.info(`Found vswhere.exe at ${vsWhereLocation} `)

        const AddToPathHelper = CreateAddToPathHelper(vsWhereLocation)
        AddToPathHelper('MSBuildPath', FindMSBuild)
        AddToPathHelper('VSTestPath', FindVSTest)
    } catch (error) {
        if (error instanceof Error) {
            core.setFailed(error.message)
        }
    }
}

await Main()
